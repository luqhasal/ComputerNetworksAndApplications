 ...
 *
 * Returns:  false if HTTP request format is not followed.  
 * Note no checks are done for the validity of any of the values
 *
 *-----------------------------------------------------------
 */
...
bool Parse_HTTP_Request(int socket, struct http_request * request_values) {
  ...
  if (version == NULL)
    version = "";
  request_values->version = malloc (strlen(version)+1);
  printf("version is: %s\n", version);
  strcpy(request_values->version, version);
