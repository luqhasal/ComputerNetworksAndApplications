...
public void updateNodeDistanceListUsingReversPoison(ArrayList<NodeDistance> newList, Node callingNode) {
     
    int callerDistance = getDistanceOfNodeInList(neighbourNodesList, callingNode.getName());
        
        //get bestNeighbour list of calling node
        newList.forEach((nodeDistance) -> {
           
            String _name = nodeDistance.getNode().getName();
            //if "this" node is in destination for neighbour "_name" then continue
            for(BestNeighbour bn: callingNode.getBestNeighoursList()){
                if(bn.getNeighbour().equalsIgnoreCase(this.getName()) && bn.getDestination().equalsIgnoreCase(_name))
                    return;
            }
            int newDistance = nodeDistance.getDistance();
            int currentDistance = getDistanceOfNodeInList(nodesDistanceList, _name);
            Graph.MessageCount++;
               if (callerDistance + newDistance < currentDistance) {
                ...
                BestNeighbour BN = this.getBestNeighbour(_name);
                if (BN == null) {
                    this.bestNeighboursList.add(new BestNeighbour(callingNode.getName(), _name, callerDistance + newDistance));
                } else {
                    if(callerDistance + newDistance < BN.getDistance())
                        BN.setNeighbour(callingNode.getName());
                    BN.setDistance(newDistance+callerDistance);
                }
                //System.out.println((callerDistance + newDistance)+" calling node : "+callingNode+" nd "+_name+" this"+getName());
                setDistanceOfNodeInList(nodesDistanceList, _name, callerDistance + newDistance);
